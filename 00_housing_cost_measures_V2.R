...
saveRDS(hcli_proportions, file = "working/data/hcli_proportions.RDS")
...
saveRDS(cost_ratios, file = "working/data/cost_ratios.RDS")
...
cr <- readRDS("working/data/cost_ratios.RDS")
cr <- cr %>% 
  mutate(region_name = str_squish(str_to_lower(str_remove(region_name, "of England"))))
